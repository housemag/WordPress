...
class RWMB_Switch_Field extends RWMB_Input_Field {
    ...
    public static function html($meta, $field) {
        ...
        $output = sprintf(
			...
			', self::render_attributes($attributes), checked(!empty($meta), 1, false)
        );
    ...
    public static function format_single_value($field, $value, $args, $post_id) {
        $on = $field['on_label'] ? $field['on_label'] : __('On', 'viral-pro');
        $off = $field['off_label'] ? $field['on_label'] : __('Off', 'viral-pro');
        return $value ? $on : $off;
    }
