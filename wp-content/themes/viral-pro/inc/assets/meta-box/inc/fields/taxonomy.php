...
class RWMB_Taxonomy_Field extends RWMB_Object_Choice_Field {
    ...
    public static function normalize($field) {
         ...
         */
        $placeholder = __('Select a term', 'viral-pro');
        $taxonomy_name = self::get_taxonomy_singular_name($field);
        if ($taxonomy_name) {
            // Translators: %s is the taxonomy singular label.
            $placeholder = sprintf(__('Select a %s', 'viral-pro'), strtolower($taxonomy_name));
        }
        $field = wp_parse_args(
                $field, array(
    ...
    public static function raw_meta($object_id, $field, $args = array()) {
        ...
        $meta = wp_get_object_terms(
                $object_id, $field['taxonomy'], array(
            'orderby' => 'term_order',
                )
        );
        if (is_wp_error($meta)) {
    ...
    public static function format_single_value($field, $value, $args, $post_id) {
        return sprintf(
                ...
                esc_url(get_term_link($value)), esc_attr($value->name), esc_html($value->name)
        );
    }
    ...
    public static function add_new_form($field) {
        ...
        $html = sprintf(
                $html, esc_html($taxonomy_object->labels->add_new_item), esc_attr($field['id']), esc_attr($taxonomy_object->labels->new_item_name)
        );
