...
class RWMB_Media_Field extends RWMB_File_Field {
    ...
    public static function admin_enqueue_scripts() {
        ...
        RWMB_Helpers_Field::localize_script_once(
                'rwmb-media', 'i18nRwmbMedia', array(
            'add' => apply_filters('rwmb_media_add_string', _x('+ Add Media', 'media', 'viral-pro')),
            'single' => apply_filters('rwmb_media_single_files_string', _x(' file', 'media', 'viral-pro')),
            'multiple' => apply_filters('rwmb_media_multiple_files_string', _x(' files', 'media', 'viral-pro')),
            'remove' => apply_filters('rwmb_media_remove_string', _x('Remove', 'media', 'viral-pro')),
            'edit' => apply_filters('rwmb_media_edit_string', _x('Edit', 'media', 'viral-pro')),
            'view' => apply_filters('rwmb_media_view_string', _x('View', 'media', 'viral-pro')),
            'noTitle' => _x('No Title', 'media', 'viral-pro'),
            'loadingUrl' => admin_url('images/spinner.gif'),
            'extensions' => self::get_mime_extensions(),
            'select' => apply_filters('rwmb_media_select_string', _x('Select Files', 'media', 'viral-pro')),
            'or' => apply_filters('rwmb_media_or_string', _x('or', 'media', 'viral-pro')),
            'uploadInstructions' => apply_filters('rwmb_media_upload_instructions_string', _x('Drop files here to upload', 'media', 'viral-pro')),
                )
        );
    }
    ...
    public static function html($meta, $field) {
        ...
        $html = sprintf(
                '<input %s data-options="%s">', self::render_attributes($attributes), esc_attr(wp_json_encode($field['js_options']))
        );
