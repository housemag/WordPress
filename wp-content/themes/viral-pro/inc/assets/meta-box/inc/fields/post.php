...
class RWMB_Post_Field extends RWMB_Object_Choice_Field {
    ...
    public static function normalize($field) {
         ...
         */
        $placeholder = __('Select a post', 'viral-pro');
        if (1 === count($field['post_type'])) {
            $post_type = reset($field['post_type']);
            $post_type_object = get_post_type_object($post_type);
            if (!empty($post_type_object)) {
                // Translators: %s is the taxonomy singular label.
                $placeholder = sprintf(__('Select a %s', 'viral-pro'), strtolower($post_type_object->labels->singular_name));
            }
        }
        $field = wp_parse_args(
                $field, array(
    ...
    public static function format_single_value($field, $value, $args, $post_id) {
        return !$value ? '' : sprintf(
                        '<a href="%s" title="%s">%s</a>', esc_url(get_permalink($value)), the_title_attribute(
                                array(
                                    'post' => $value,
                                    'echo' => false,
                                )
