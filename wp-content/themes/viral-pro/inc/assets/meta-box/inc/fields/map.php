...
class RWMB_Map_Field extends RWMB_Field {
    ...
    public static function admin_enqueue_scripts() {
        ...
        wp_enqueue_script(
                'rwmb-map', RWMB_JS_URL . 'map.js', array(
            'jquery-ui-autocomplete',
            'google-maps',
                ), RWMB_VER, true
        );
        RWMB_Helpers_Field::localize_script_once(
                'rwmb-map', 'RWMB_Map', array(
            'no_results_string' => __('No results found', 'viral-pro'),
                )
        );
    }
    ...
    public static function html($meta, $field) {
        ...
        $html = sprintf(
                '<div class="rwmb-map-field" data-address-field="%s">', esc_attr($address)
        );

        $html .= sprintf(
                '<div class="rwmb-map-canvas" data-default-loc="%s" data-region="%s"></div>
			<input type="hidden" name="%s" class="rwmb-map-coordinate" value="%s">', esc_attr($field['std']), esc_attr($field['region']), esc_attr($field['field_name']), esc_attr($meta)
        );
    ...
    public static function render_map($location, $args = array()) {
        ...
        $output = sprintf(
                '<div class="rwmb-map-canvas" data-map_options="%s" style="width:%s;height:%s"></div>', esc_attr(wp_json_encode($args)), esc_attr($args['width']), esc_attr($args['height'])
        );
        return $output;
    }
