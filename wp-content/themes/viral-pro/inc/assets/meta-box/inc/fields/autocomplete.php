...
class RWMB_Autocomplete_Field extends RWMB_Multiple_Values_Field {
    ...
    public static function admin_enqueue_scripts() {
        ...
        RWMB_Helpers_Field::localize_script_once(
                'rwmb-autocomplete', 'RWMB_Autocomplete', array(
            'delete' => __('Delete', 'viral-pro'),
                )
        );
    }
    ...
    public static function html($meta, $field) {
        ...
        $html = sprintf(
			...
			<input type="hidden" name="%s" class="rwmb-autocomplete" data-options="%s" disabled>', esc_attr($field['size']), esc_attr($field['field_name']), esc_attr($options)
        );
        ...
        if (is_array($field['options'])) {
            foreach ($field['options'] as $value => $label) {
                ...
                $html .= sprintf(
                        $tpl, esc_html($label), esc_html__('Delete', 'viral-pro'), esc_attr($field['field_name']), esc_attr($value)
                );
            }
        } else {
