...
class RWMB_OSM_Field extends RWMB_Field {
    ...
    public static function html($meta, $field) {
        ...
        $html = sprintf(
                '<div class="rwmb-osm-field" data-address-field="%s">', esc_attr($address)
        );

        $html .= sprintf(
                '<div class="rwmb-osm-canvas" data-default-loc="%s" data-region="%s" data-language="%s"></div>
			<input type="hidden" name="%s" class="rwmb-osm-coordinate" value="%s">', esc_attr($field['std']), esc_attr($field['region']), esc_attr($field['language']), esc_attr($field['field_name']), esc_attr($meta)
        );
    ...
    public static function render_map($location, $args = array()) {
        ...
        $output = sprintf(
                '<div class="rwmb-osm-canvas" data-osm_options="%s" style="width:%s;height:%s"></div>', esc_attr(wp_json_encode($args)), esc_attr($args['width']), esc_attr($args['height'])
        );
        return $output;
    }
