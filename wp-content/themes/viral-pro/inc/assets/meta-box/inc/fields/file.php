...
class RWMB_File_Field extends RWMB_Field {
    ...
    public static function admin_enqueue_scripts() {
        ...
        RWMB_Helpers_Field::localize_script_once(
            ...
            'maxFileUploadsSingle' => __('You may only upload maximum %d file', 'viral-pro'),
            // Translators: %d is the number of files in plural form.
            'maxFileUploadsPlural' => __('You may only upload maximum %d files', 'viral-pro'),
                )
        );
    }
    ...
    public static function ajax_delete_file() {
        ...
        if (!in_array($attachment, $field_value)) {
            wp_send_json_error(__('Error: Invalid file', 'viral-pro'));
        }
        ...
        wp_send_json_error(__('Error: Cannot delete file', 'viral-pro'));
    }
    ...
    public static function html($meta, $field) {
        ...
        $i18n_more = apply_filters('rwmb_file_add_string', _x('+ Add new file', 'file upload', 'viral-pro'), $field);
        ...
        // Upload new files.
        $html .= sprintf(
                '<div class="rwmb-file-new"><input %s>', self::render_attributes($attributes)
        );
        if (1 !== $field['max_file_uploads']) {
            $html .= sprintf(
                    '<a class="rwmb-file-add" href="#"><strong>%s</strong></a>', $i18n_more
            );
        }
        $html .= '</div>';

        $html .= sprintf(
                '<input type="hidden" class="rwmb-file-index" name="%s" value="%s">', $field['index_name'], $field['input_name']
        );
    ...
    protected static function get_uploaded_files($files, $field) {
        ...
        return sprintf(
                '<ul class="rwmb-uploaded" data-field_id="%s" data-delete_nonce="%s" data-reorder_nonce="%s" data-force_delete="%s" data-max_file_uploads="%s" data-mime_type="%s">%s</ul>', $field['id'], $delete_nonce, $reorder_nonce, $field['force_delete'] ? 1 : 0, $field['max_file_uploads'], $field['mime_type'], $output
        );
    }
    ...
    protected static function file_html($file, $index, $field) {
        $i18n_delete = apply_filters('rwmb_file_delete_string', _x('Delete', 'file upload', 'viral-pro'));
        $i18n_edit = apply_filters('rwmb_file_edit_string', _x('Edit', 'file upload', 'viral-pro'));
        ...
        return sprintf(
			...
			</li>', $data['url'], $data['icon'], $data['url'], $data['title'], $data['name'], $data['edit_link'], $file, $i18n_delete, $attributes['name'], $index, $file
        );
    }
