...
class RWMB_Update_Settings {
    ...
    public function add_settings_page() {
        $parent = $this->option->is_network_activated() ? 'settings.php' : 'viral-pro';
        $capability = $this->option->is_network_activated() ? 'manage_network_options' : 'manage_options';
        $title = $this->option->is_network_activated() ? esc_html__('Meta Box License', 'viral-pro') : esc_html__('License', 'viral-pro');
        $page_hook = add_submenu_page(
                $parent, $title, $title, $capability, 'meta-box-updater', array($this, 'render')
        );
    ...
    public function render() {
        ?>
        <div class="wrap">
            <h1><?php esc_html_e('Meta Box License', 'viral-pro'); ?></h1>
            <p><?php esc_html_e('Please enter your license key to enable automatic updates for Meta Box extensions.', 'viral-pro'); ?></p>
            <p>
                <?php
                printf(
                        // Translators: %1$s - URL to the My Account page, %2$s - URL to the pricing page.
                        wp_kses_post(__('To get the license key, visit the <a href="%1$s" target="_blank">My Account</a> page on metabox.io website. If you have not purchased any extension yet, please <a href="%2$s" target="_blank">get a new license here</a>.', 'viral-pro')), 'https://metabox.io/my-account/', 'https://metabox.io/pricing/'
                );
                ?>
            </p>

            <form action="" method="post">
                <?php wp_nonce_field('viral-pro'); ?>

                <table class="form-table">
                    <tr>
                        <th scope="row"><?php esc_html_e('License Key', 'viral-pro'); ?></th>
                        <td>
                            <?php
                            $messages = array(
                                // Translators: %1$s - URL to the pricing page.
                                'invalid' => __('Your license key is <b>invalid</b>. Please update your license key or <a href="%1$s" target="_blank">get a new one here</a>.', 'viral-pro'),
                                // Translators: %1$s - URL to the pricing page.
                                'error' => __('Your license key is <b>invalid</b>. Please update your license key or <a href="%1$s" target="_blank">get a new one here</a>.', 'viral-pro'),
                                // Translators: %2$s - URL to the My Account page.
                                'expired' => __('Your license key is <b>expired</b>. Please <a href="%2$s" target="_blank">renew your license</a>.', 'viral-pro'),
                                'active' => __('Your license key is <b>active</b>.', 'viral-pro'),
                            );
                ...
                <?php submit_button(__('Save Changes', 'viral-pro')); ?>
            </form>
        </div>
        <?php
    }
    ...
    public function save() {
        ...
        if (false === $response) {
            add_settings_error('', 'mb-error', __('Something wrong with the connection to metabox.io. Please try again later.', 'viral-pro'));
        } elseif ('active' === $status) {
            add_settings_error('', 'mb-success', __('Your license is activated.', 'viral-pro'), 'updated');
        } elseif ('expired' === $status) {
            // Translators: %s - URL to the My Account page.
            $message = __('License expired. Please renew on the <a href="%s" target="_blank">My Account</a> page on metabox.io website.', 'viral-pro');
            $message = wp_kses_post(sprintf($message, 'https://metabox.io/my-account/'));

            add_settings_error('', 'mb-expired', $message);
        } else {
            // Translators: %1$s - URL to the My Account page, %2$s - URL to the pricing page.
            $message = __('Invalid license. Please <a href="%1$s" target="_blank">check again</a> or <a href="%2$s" target="_blank">get a new license here</a>.', 'viral-pro');
