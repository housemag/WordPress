...
abstract class RWMB_Field {
    ...
    public static function show($field, $saved, $post_id = 0) {
        ...
        $outer_html = sprintf(
                $field['before'] . '<div class="%s">%s</div>' . $field['after'], esc_attr(trim($classes)), $html
        );
    ...
    public static function begin_html($meta, $field) {
        ...
        if ($field['name']) {
            $field_label = sprintf(
					...
					<label for="%s">%s</label>
					%s
				</div>', esc_attr($field['id']), $field['name'], self::label_description($field)
            );
        }
        ...
        $input_open = sprintf(
                '<div class="rwmb-input"%s>', $data_max_clone
        );
    ...
    public static function normalize($field) {
        $field = wp_parse_args(
            ...
            'max_clone' => 0,
            'sort_clone' => false,
            'add_button' => __('+ Add more', 'viral-pro'),
            'clone_default' => false,
