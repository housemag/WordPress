...
class RW_Meta_Box {
    ...
    public function add_meta_boxes() {
        ...
        foreach ($this->post_types as $post_type) {
            add_meta_box(
                    $this->id, $this->title, array($this, 'show'), $post_type, $this->context, $this->priority
            );
        }
    }
    ...
    public function show() {
        ...
        // Container.
        printf(
                '<div class="%s" data-autosave="%s" data-object-type="%s" data-object-id="%s">', esc_attr(trim("rwmb-meta-box {$this->class}")), esc_attr($this->autosave ? 'true' : 'false' ), esc_attr($this->object_type), esc_attr($this->object_id)
        );
    ...
    public function validate() {
        ...
        return !$this->saved && (!defined('DOING_AUTOSAVE') || $this->autosave ) && wp_verify_nonce($nonce, "rwmb-save-{$this->id}");
    }
    ...
    public function is_saved() {
        foreach ($this->fields as $field) {
            ...
            if (
                    (!$field['multiple'] && '' !== $value ) || ( $field['multiple'] && is_array($value) && array() !== $value )
            ) {
                return true;
            }
        }
