 ...
 * @package Viral Pro
 */
add_action('widgets_init', 'viral_pro_register_post_tab');

function viral_pro_register_post_tab() {
    register_widget('viral_pro_post_tab');
}

class viral_pro_post_tab extends WP_Widget {

    public function __construct() {
        $widget_ops = array('description' => __('A widget to display latest, most commented, most viewed post with thumbnail in tab.', 'viral-pro'));
        parent::__construct('viral_pro_post_tab', '&bull; VP : Post Tab', $widget_ops);
    }
    ...
    private function widget_fields() {
        $fields = array(
            ...
            'tabs' => array(
                ...
                'viral_pro_widgets_title' => __('Add Tabs', 'viral-pro'),
                'viral_pro_widgets_repeater_title' => __('Tab', 'viral-pro'),
                ...
                'viral_pro_widgets_repeater_fields' => array(
                    ...
                    'category' => array(
                        ...
                        'desc' => __('If Category is selected for Tab Content.', 'viral-pro')
                    )
                ),
                'viral_pro_widgets_add_button' => __('Add New Tab', 'viral-pro')
            ),
            'post_count' => array(
    ...
    public function widget($args, $instance) {
        ...
        $title_html_tag = isset($instance['title_html_tag']) ? $instance['title_html_tag'] : 'default';
        ...
        $tab_header = $tab_content = $title_style = $amp_content = $excerpt_style = "";
        $class = 'ht-pl-title';
        ...
        <?php echo viral_pro_is_amp() ? '<amp-selector class="ht-post-tab" role="tablist" keyboard-select-mode="focus">' : '<div class="ht-post-tab">'; ?>
        <?php
        if (!empty($tabs)) {
            foreach ($tabs as $key => $tab) {
                $id = uniqid('ht-pt-tab-', false);
                $tab_header_data = viral_pro_is_amp() ? ('<div id="' . $id . '" role="tab" class="ht-pt-tab" aria-controls="tabpanel' . $id . '" option ' . ($key == 1 ? 'selected="selected"' : '') . '>') : ('<div class="ht-pt-tab" data-id="' . $id . '">');
                $tab_header_data .= $tab['title'];
                $tab_header_data .= '</div>';
                $tab_header .= $tab_header_data;
                ...
                if (isset($tab['content']) && $tab['content'] == 'most-commented') {
                    $args['orderby'] = 'comment_count';
                }

                if (isset($tab['content']) && $tab['content'] == 'most-viewed') {
                    $args['orderby'] = 'meta_value_num';
                    $args['meta_key'] = 'post_views_count';
                }

                if (isset($tab['content']) && $tab['content'] == 'category') {
                    $args['cat'] = isset($tab['category']) ? $tab['category'] : '';
                }

                $query = new WP_Query($args);
                $tab_content_data = viral_pro_is_amp() ? ('<div id="tabpanel' . $id . '" role="tabpanel" class="ht-pt-content" aria-labelledby="' . $id . '">') : ('<div class="ht-pt-content" id="' . $id . '">');
                $tab_content_data .= '<div class="ht-post-listing animated faster ' . esc_attr($layout) . '">';
                ob_start();
                while ($query->have_posts()) : $query->the_post();
                    ...
                    <div class="ht-post-list ht-clearfix">
                        <?php
                        $image_size = ($layout == 'style4' && $index != 1) ? 'viral-pro-150x150' : $thumbnail_size;
                        ?>
                ...
                endwhile;
                wp_reset_postdata();
                $tab_content_data .= ob_get_contents();
                ob_end_clean();
                $tab_content_data .= '</div>';
                $tab_content_data .= '</div>';
                $tab_content .= $tab_content_data;
                $amp_content .= viral_pro_is_amp() ? $tab_header_data . $tab_content_data : '';
            }
        }

        echo viral_pro_is_amp() ? $amp_content : ('<div class="ht-pt-header">' . $tab_header . '</div>' . '<div class="ht-pt-content-wrap">' . $tab_content . '</div>');
        echo viral_pro_is_amp() ? '</amp-selector>' : '</div>';
        echo $after_widget;
    }
