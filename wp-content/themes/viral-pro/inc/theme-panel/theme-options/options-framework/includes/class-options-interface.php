...
class Options_Framework_Interface {
    ...
    static function optionsframework_fields() {
        ...
        foreach ($options as $value) {
            ...
            // Wrap all options
            if (( $value['type'] != "heading" ) && ( $value['type'] != "info") && ( $value['type'] != "break" )) {
                ...
                if ($value['type'] != 'editor') {
                    $output .= '<div class="option">' . "\n" . '<div class="controls wp-clearfix">' . "\n";
                } else {
                    $output .= '<div class="option">' . "\n" . '<div>' . "\n";
                }
            }
            ...
            switch ($value['type']) {
                ...
                // Checkbox
                case "checkbox":
                    $output .= '<div class="checkbox-wrap wp-clearfix">';
                    $output .= '<div class="onoff-switch">';
                    $output .= '<input id="' . esc_attr($value['id']) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr($option_name . '[' . $value['id'] . ']') . '" ' . checked($val, 1, false) . ' />';
                    $output .= '<label class="onoff-switch-label" for="' . esc_attr($value['id']) . '"></label>';
                    $output .= '</div>';
                    if (isset($value['label'])) {
                        $output .= '<label for="' . esc_attr($value['id']) . '">' . wp_kses($value['label'], $allowedtags) . '</label>';
                    }
                    $output .= '</div>';
                    break;
                ...
                case "multicheck":
                    foreach ($value['options'] as $key => $option) {
                        ...
                        $output .= '<div class="checkbox-wrap wp-clearfix">';
                        $output .= '<div class="onoff-switch">';
                        $output .= '<input id="' . esc_attr($id) . '" class="checkbox of-input" type="checkbox" name="' . esc_attr($name) . '" ' . $checked . '/>';
                        $output .= '<label class="onoff-switch-label" for="' . esc_attr($id) . '"></label>';
                        $output .= '</div>';
                        $output .= '<label for="' . esc_attr($id) . '">' . wp_kses($label, $allowedtags) . '</label>';
                        $output .= '</div>';
                    }
                    break;
