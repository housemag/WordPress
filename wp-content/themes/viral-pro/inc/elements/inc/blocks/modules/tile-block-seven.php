...
class Viral_Pro_Tile_Block_Seven extends Viral_Pro_Posts_Block {
    ...
    public function inner($posts, $settings) {
        ...
        $output = '';
        $output .= '<div class="vl-tile-block ht-clearfix style7 ' . esc_attr($gap) . '">';
        if (!empty($posts)) {
    ...
    public function render_content($settings) {
        ...
        $class = $index == 3 ? 'vl-width-50 vl-height-100 vl-thumb vl-featured-post' : 'vl-width-100 vl-height-50 vl-thumb';
        $title_class = $index == 3 ? 'vl-large-title vl-post-title' : 'vl-post-title';

        ob_start();
        if ($index == 1 || $index == 4) {
            echo '<div class="vl-width-25 vl-height-100 vl-parent vl-side-post">';
        }
        ?>  
        <div class="<?php echo esc_attr($class); ?>">
            <div class="vl-thumb-inner vl-post-thumb vp-module <?php echo join(' ', get_post_class('', $source->post_ID)); ?>">
                <a href="<?php echo $source->get_permalink(); ?>">
                    <?php
                    if ($index == 3)
                        echo $source->get_featured_image($featured_image_size);
                    else
                        echo $source->get_featured_image($side_image_size);
                    ?>
                </a>
                ...
                <div class="vl-title-container <?php echo esc_attr($content_align_class); ?>">
                    <a href="<?php echo $source->get_permalink(); ?>">
                        <h3 class="<?php echo esc_attr($title_class) ?>"><span><?php echo $source->get_title(); ?></span></h3>
    ...
    protected function get_post_meta($settings, $count) {
        $source = $this->source;
        $post_author = $count == 3 ? $settings['featured_post_author'] : $settings['side_post_author'];
        $post_date = $count == 3 ? $settings['featured_post_date'] : $settings['side_post_date'];
        $post_comment = $count == 3 ? $settings['featured_post_comment'] : $settings['side_post_comment'];
