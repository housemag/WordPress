...
class Viral_Pro_News_Block_Twentytwo extends Viral_Pro_Posts_Block {
    ...
    public function inner($posts, $settings) {
        $output = '';
        $output .= '<div class="vl-double-block">';
        if (!empty($posts)) {
    ...
    public function render_content($settings) {
        ...
        if ($index == 1 || $index == 2) {
            ...
            <?php
            if ($index == 2) {
                echo '</div>'; // close big-block div when second post
            }
        } else {
            if ($index == 3) {
                echo '<div class="vl-small-block">';
            }
            ?>
            <div class="vl-post-item ht-clearfix vp-module <?php echo join(' ', get_post_class('', $source->post_ID)); ?>">
                <div class="vl-post-thumb">
                ...
                <div class="vl-post-content">
                    ...
                    <?php
                    if ($index > 2) {
                        $this->get_post_excerpt($settings, $index);
                    }
                    ?>
                </div>
            </div>
            <?php
            if ($index == $last) {
                echo '</div>'; // close small-block div when last post
            }
        }
        $output = ob_get_clean();
    ...
    protected function get_block_class() {
        return 'vl-news-block style5';
    }

    protected function get_post_excerpt($settings, $count) {
        $source = $this->source;
        $excerpt_length = $count > 2 ? $settings['listing_excerpt_length'] : $settings['featured_excerpt_length'];
        if ($excerpt_length) {
            ?>
    ...
    protected function get_post_meta($settings, $count) {
        $source = $this->source;
        $post_author = ($count == 1 || $count == 2) ? $settings['featured_post_author'] : $settings['listing_post_author'];
        $post_date = ($count == 1 || $count == 2) ? $settings['featured_post_date'] : $settings['listing_post_date'];
        $post_comment = ($count == 1 || $count == 2) ? $settings['featured_post_comment'] : $settings['listing_post_comment'];
