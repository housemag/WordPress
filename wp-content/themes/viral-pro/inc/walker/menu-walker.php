 ...
 *
 */
if (!class_exists('Viral_Pro_Custom_Nav_Walker')) {

    class Viral_Pro_Custom_Nav_Walker extends Walker_Nav_Menu {
        ...
        public function start_lvl(&$output, $depth = 0, $args = array()) {
            ...
            if ($depth === 0 && $this->megamenu != '' && $this->megamenu != 'normal') {
        ...
        public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
            ...
            // Mega menu
            if ($depth === 0 && $this->has_children && $item->category_post == '') {
            ...
            if ($depth === 0 && $item->category_post != '' && $item->object == 'category') {
            ...
            $item_output .= '</a>';
            if ($depth !== 0 && $item->megamenu_template && $this->megamenu != 'normal') {
                ob_start();
            ...
            if ($depth !== 0 && $item->megamenu_widgetarea && $this->megamenu != 'normal') {
                ob_start();
            ...
            if ($depth === 0 && $item->category_post != '') {
                ...
                if ($item->category_post != '' && $item->object == 'category') {
                    ...
                    foreach ($sub_categories as $category) {
                        ...
                        $cat_content .= '<ul class="ht-clearfix" id="mm-cat-' . $item->ID . $cat_id . '">';
                        while ($cat_query->have_posts()) {

                            $cat_query->the_post();
                            if (has_post_thumbnail()) {
                                $viral_pro_image = wp_get_attachment_image_src(get_post_thumbnail_id(), 'viral-pro-360x240');
                                $viral_pro_image_url = $viral_pro_image[0];
                            } else {
                                $viral_pro_image_url = get_template_directory_uri() . '/images/placeholder-360x240.jpg';
                            }

                            $cat_content .= '<li class="megamenu-category-post">';

                            $cat_content .= '<a href="' . get_permalink() . '" class="mega-post-link">';

                            $cat_content .= '<img src="' . esc_url($viral_pro_image_url) . '" alt="' . esc_attr(get_the_title()) . '">';

                            $cat_content .= '</a>';

                            $cat_content .= '<h3 class="mega-post-title vl-post-title"><a href="' . get_permalink() . '">' . get_the_title() . '</a></h3>';
        ...
        public function display_element($element, &$children_elements, $max_depth, $depth = 0, $args = array(), &$output = '') {
            ...
            // Down Arrows
            if (($depth == 0 && !empty($children_elements[$element->$id_field])) || ( $depth == 0 && $element->category_post != '' && $element->object == 'category')) {
                $element->title .= '<span class="dropdown-nav ei arrow_carrot-down"></span>';
            }

            // Right/Left Arrows
            if (!empty($children_elements[$element->$id_field]) && ( $depth > 0 )) {
                if (is_rtl()) {
                    $element->title .= '<span class="dropdown-nav ei arrow_carrot-right"></span>';
                } else {
                    $element->title .= '<span class="dropdown-nav ei arrow_carrot-right"></span>';
                }
            }
